...
const AUDIT_PATH = path.join(AUDIT_DIR, 'v3_5_beta_seed_asset_media_pixabay_sources.json');
...
function pickMediaUrl(asset, kindIndex) {
  const pool = PIXABAY_POOLS[asset.category] || PIXABAY_POOLS[asset.assetClass];
  if (!pool?.length) {
    throw new Error(`No Pixabay pool configured for ${asset.assetUid} category=${asset.category} assetClass=${asset.assetClass}`);
  }
  const profileNumber = Number.parseInt(String(asset.sourceProfileId || '').replace(/\D/g, ''), 10) || 1;
  const assetNumber = Number.parseInt(String(asset.assetUid || '').replace(/.*asset-/, ''), 10) || 1;
  return pool[(profileNumber + assetNumber + kindIndex) % pool.length];
}
...
function buildRecord(asset, target, sourceUrl, fileSize = null) {
  return {
...
async function main() {
  ...
  for (const asset of assets) {
    mediaTargets(asset).forEach((target, index) => {
      planned.push(buildRecord(asset, target, pickMediaUrl(asset, index)));
    });
  }
  ...
  for (const record of planned) {
    ...
    console.log(`${record.assetUid} ${record.kind} <- ${record.sourceUrl}`);
