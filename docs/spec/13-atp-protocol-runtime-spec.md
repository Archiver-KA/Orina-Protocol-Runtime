...
World Chain Sepolia is a separately deployed ATP v3.5 testnet stack on chain `4801`, deployed under namespace `orina-atp-v3.5-worldchain-sepolia-eoa-testnet-20260702`. It uses deployer EOA `0x282Be18838D7079C215F49749a9606d77e00888b` as the timelock proposer, executor, canceller, and admin with zero delay, matching the temporary Sepolia/Fuji testnet paths. This is testnet-only; World Chain mainnet chain `480` must redeploy with a production multisig/Safe, non-zero timelock delay, and a new address set.
...
On July 2, 2026, `MarketplaceATP.delegationManager()` returned `0x5e41f1155AB4E614037C9C481BB8c1d398915cd0` on World Chain Sepolia. The first World Chain M2M pair `0x52440e44ec34a64e19b92243262fe47819d65539` / `0x7D6b498eDc3F469ED020116e8892EbB361753bCB` is orphaned because it was deployed with the stale Base Sepolia timelock and is not the runtime address set. The runtime verification gate checks `MarketplaceATP.VERSION == 3.4`, `FeeManager.VERSION == 3.5`, `PaymentGateway.VERSION == 3.5`, starter-kit token metadata, timelock delay, and World Chain Sepolia M2M roles.

### Governance and fee endpoints

| Role / endpoint | Address | Notes |
| --- | --- | --- |
| BSC/Base governance safe | `0x554c4F489846e293bA251fb8B863FE1241306138` | Not the Arbitrum, Ethereum, Optimism, Avalanche Fuji, or World Chain Sepolia signer. |
...
| World Chain Sepolia testnet timelock | `0x5C842728C357B9b18eb8A9A7a840499936132e67` | Owns Marketplace governance and active DelegationManager admin for this testnet address set. |
| World Chain Sepolia testnet operator EOA | `0x282Be18838D7079C215F49749a9606d77e00888b` | Timelock admin, proposer, executor, and canceller with zero delay for testnet only. |
