...
Seller confirmation is a NON-BINDING intent signal.
...
- finalized: bool
- proposedAt: uint256
- sellerConfirmedAt: uint256 (optional)
- paidAt: uint256 (optional)
- disputeOpenedAt: uint256 (optional)
...
- PENDING_CONFIRM:
  Order exists, no escrow, no asset lock.
...
Effects:
- orders[orderId].sellerConfirmedAt = block.timestamp
...
Seller confirmation is informational only.
...
- PaymentGateway escrows buyer funds
...
- buyer payment deadline (indirectly)
...
payDeadline is computed at createOrder():

- payDeadline
  = proposedAt + SELLER_CONFIRM_WINDOW + PAY_TIMEOUT

Authoritative behavior:
- Buyer payment window STARTS at order creation
- Seller confirmation delay REDUCES buyer’s effective payment time

This is intentional per code, even if non-ideal UX.

Spec correction:
- Buyer payment window does NOT begin at sellerConfirm
- Seller confirmation does NOT shift payDeadline
...
- Buyer may receive less than PAY_TIMEOUT
...
- Buyer always has PAY_TIMEOUT after sellerConfirm
- autoRelease waits for dispute window end
- sellerConfirm shifts payDeadline
...
- initial payDeadline computation
...
payDeadline is computed ONCE at createOrder():

payDeadline :=
  proposedAt
+ SELLER_CONFIRM_WINDOW
+ PAY_TIMEOUT

Key properties:

- payDeadline does NOT shift on sellerConfirm
- buyer pay window may be shorter if seller confirms late
- this is INTENTIONAL per current code

Spec MUST NOT claim:
- “buyer pay window starts after seller confirm”
- “buyer always has full PAY_TIMEOUT”
...
This window may be < PAY_TIMEOUT.
...
Only addresses with AUTOTIME_ROLE may:

- autoRelease(orderId)
- cancelOrder(orderId)

AUTOTIME_ROLE is granted ONCE in constructor.

--------------------------------------------------
9.10 Auto-Cancel Semantics
--------------------------------------------------

Auto-cancel is allowed iff:

- state == PENDING_CONFIRM
- block.timestamp >= proposedAt + SELLER_CONFIRM_WINDOW
- caller == autoTimeManager
...
- “Buyer pay window starts after seller confirm”
- “Buyer always has PAY_TIMEOUT after confirm”
- “Deadlines shift dynamically”

The following is AUTHORITATIVE:

- payDeadline is absolute and fixed at creation.
