-- ATP2 Batch C18 / profile reputation summary precision fix
-- Scope:
--   - prevent numeric overflow when protocol order totals are stored at very large precision
--   - keep the existing profile_reputation_summaries contract intact for clients

create or replace view public.profile_reputation_summaries as
...
base as (
  select
    ...
    coalesce(om.total_volume, 0)::numeric as total_volume,
    case
