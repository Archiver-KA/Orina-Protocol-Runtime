begin;

do $$
declare
  v_target_count integer;
  v_updated_count integer;
begin
  select count(*)
  into v_target_count
  from public.assets_catalog
  where asset_uid like 'v35-seed-p%-asset-%'
    and metadata ->> 'source_profile_id' ~ '^P[0-9]{3}$'
    and substring(metadata ->> 'source_profile_id' from 2)::integer between 51 and 100
    and metadata ->> 'currency' = 'USDC';

  if v_target_count <> 150 then
    raise exception 'Dual-token rollback preflight failed: expected 150 USDC assets, found %', v_target_count;
  end if;

  update public.assets_catalog
  set
    attributes = jsonb_set(
      attributes,
      '{estimated_price,currency}',
      to_jsonb('USDT'::text),
      true
    ),
    metadata = (
      jsonb_set(
        jsonb_set(
          metadata,
          '{currency}',
          to_jsonb('USDT'::text),
          true
        ),
        '{price}',
        to_jsonb(regexp_replace(coalesce(metadata ->> 'price', ''), '\s+USDC$', ' USDT')),
        true
      ) - 'dual_token_policy'
    ),
    updated_at = now()
  where asset_uid like 'v35-seed-p%-asset-%'
    and metadata ->> 'source_profile_id' ~ '^P[0-9]{3}$'
    and substring(metadata ->> 'source_profile_id' from 2)::integer between 51 and 100
    and metadata ->> 'currency' = 'USDC';

  get diagnostics v_updated_count = row_count;
  if v_updated_count <> 150 then
    raise exception 'Dual-token rollback failed: expected 150 updates, got %', v_updated_count;
  end if;
end
$$;

refresh materialized view public.marketplace_asset_browse_index_v1;

commit;
