...
if (!fs.existsSync(envPath)) {
  console.error('[run_review_smoke_from_env] Missing .env');
  process.exit(1);
}
...
if (!baseUrl || !anonKey) {
  console.error('[run_review_smoke_from_env] Missing VITE_SUPABASE_URL or anon key in .env');
  process.exit(1);
}

const scriptPath = path.resolve(process.cwd(), 'supabase', 'audit', 'smoke_review_end_to_end.cjs');
