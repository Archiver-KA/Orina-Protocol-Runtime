-- ATP2 Batch 4C RLS Temp Client-Write Audit Snapshot (single-result)
-- Purpose: verify temporary unblock policies for profiles/community tables (no messaging).
-- SQL Editor note: returns ONE result table.
-- Invariant: format batch: pham vi hep, checklist chot ro, test sau tung buoc.

drop table if exists pg_temp._atp2_batch4c_rls_audit;

create temporary table _atp2_batch4c_rls_audit (
  seq integer primary key,
  section text not null,
  payload jsonb not null
);

insert into _atp2_batch4c_rls_audit (seq, section, payload)
select
  0,
  'context',
  jsonb_build_object(
...
select
  1,
  'batch4c_profiles_community_rls_enabled',
  jsonb_build_object(
...
select
  2,
  'batch4c_owner_scoped_deferred_rls_still_disabled',
  jsonb_build_object(
...
select
  3,
  'batch4c_temp_client_write_policies_presence',
  jsonb_build_object(
...
select
  4,
  'batch4c_temp_client_write_policy_snapshot',
  coalesce(
    jsonb_agg(
      jsonb_build_object(
        'tablename', tablename,
...
select
  6,
  'batch4a_read_policies_still_present_subset',
  jsonb_build_object(
    'present', coalesce((select jsonb_agg(policyname order by policyname) from present), '[]'::jsonb),
    'missing', coalesce((select jsonb_agg(policyname order by policyname) from missing), '[]'::jsonb)
  );
