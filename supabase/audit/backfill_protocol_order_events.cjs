...
const DEFAULTS = {
  ...
  marketplace: '0x6154d16f4f52c1a4157928f136a53ac3b83b510b',
  disputeManager: '0x33dceb1e8aec7fe69d8a1390de0cc0e879035949',
...
async function buildMissingProtocolOrderRows({
  ...
  for (const orderUid of missingOrderUids) {
    ...
    const row = {
      ...
      asset_token_id: String(result[3]),
      buyer_address: normalizeAddress(result[0]),
      seller_address: normalizeAddress(result[1]),
      status: normalizeBackfilledOrderStatus({
        state: result[11],
        finalized: result[18],
        sellerConfirmed: result[19],
      }),
      amount: String(result[4]),
      price_per_unit:
        result[4] > 0n
          ? String(result[5] / result[4])
          : String(result[5]),
      total_value: String(result[5]),
      currency_symbol: normalizeAddress(result[2]),
      metadata: {
        ...
        chainSnapshot: {
          proposedAt: String(result[6]),
          paidAt: String(result[7]),
          autoReleaseAt: String(result[8]),
          estDeliverySeconds: String(result[9]),
          payDeadline: String(result[10]),
          state: Number(result[11]),
          settlementType: Number(result[12]),
          split: {
            buyerShareBps: String(result[13]?.buyerShareBps ?? 0n),
            sellerShareBps: String(result[13]?.sellerShareBps ?? 0n),
          },
          platformFeeBpsSnapshot: String(result[14]),
          daoFeeBpsSnapshot: String(result[15]),
          burnFeeBpsSnapshot: String(result[16]),
          referralFeeBpsSnapshot: String(result[17]),
          finalized: Boolean(result[18]),
          sellerConfirmed: Boolean(result[19]),
          buyerSig1Present: Boolean(result[20] && result[20] !== '0x'),
          sellerSigPresent: Boolean(result[21] && result[21] !== '0x'),
          buyerSig2Present: Boolean(result[22] && result[22] !== '0x'),
        },
      },
    };
    rows.push(row);
...
function runApplyLinked(sqlFile) {
  const result = spawnSync(
    'cmd.exe',
    ['/c', 'npx', 'supabase', 'db', 'query', '--linked', '--file', sqlFile],
    {
      cwd: ROOT,
      encoding: 'utf8',
