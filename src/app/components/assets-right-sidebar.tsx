    ...
    <StudioSidebarShell widthClassName="w-full" className="bg-ui-page border-l-0 p-2.5">
      <div className="h-full rounded-[24px] bg-[var(--t-card-bg)] backdrop-blur-[6px] flex flex-col overflow-hidden">
        <StudioSidebarHeader className="p-5 border-b border-[var(--t-border-subtle)]">
          ...
          <p className="text-xs text-ui-muted mt-1">Canonical wallet inventory, categories, and value ranking</p>
        </StudioSidebarHeader>
        ...
        <StudioSidebarScroll className="p-4 space-y-4">
            ...
            <div className="space-y-1.5">
              {categoryEntries.length === 0 && (
                <StudioPanel className="rounded-xl px-3.5 py-3 text-[10px] text-ui-muted">
                  No canonical asset categories indexed for this wallet yet.
                </StudioPanel>
              )}
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-[10px] font-medium text-ui-secondary">{category.name}</span>
                    </div>
              ...
              {topValuedAssets.map((asset, index) => (
                ...
                >
                  <StudioListItem
                    ...
                    right={<span className="text-[10px] font-bold text-ui-primary whitespace-nowrap">{asset.valueLabel}</span>}
                  />
                </StudioPanel>
              ))}
            </div>
          ...
          <StudioPanel className="rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-[11px] uppercase font-bold text-ui-muted tracking-wider">Listing Status</h3>
              <Fuel className="text-primary" size={14} />
            </div>
            <div className="space-y-3">
              <StudioMetricRow
                left={<div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-[#2CC295] animate-pulse" /><span className="text-[10px] font-medium text-ui-secondary">Active RWA</span></div>}
                right={<span className="text-[10px] font-bold text-primary">{portfolio.listingStatus.activeRwa}</span>}
              />
              <StudioMetricRow
                left={<div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-red-400" /><span className="text-[10px] font-medium text-ui-secondary">Sold Out RWA</span></div>}
                right={<span className="text-[10px] font-bold text-red-400">{portfolio.listingStatus.soldOutRwa}</span>}
              />
              <StudioMetricRow
                left={<div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-orange-400" /><span className="text-[10px] font-medium text-ui-secondary">Non-Transferable Receipts</span></div>}
                right={<span className="text-[10px] font-bold text-orange-400">{portfolio.listingStatus.nonTransferableReceipts}</span>}
              />
              <StudioMetricRow
                left={<div className="flex items-center gap-2"><div className="w-1.5 h-1.5 rounded-full bg-blue-400" /><span className="text-[10px] font-medium text-ui-secondary">Transferable NFTs</span></div>}
                right={<span className="text-[10px] font-bold text-blue-400">{portfolio.listingStatus.transferableNfts}</span>}
              />
            </div>
          </StudioPanel>
          ...
          <StudioPanel className="rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-[11px] uppercase font-bold text-ui-muted tracking-wider">Wallet Snapshot</h3>
              <Sparkles className="text-primary" size={14} />
            </div>
            <div className="space-y-3">
              <StudioMetricRow
                left={<span className="text-[10px] font-medium text-ui-secondary">Total Portfolio</span>}
                right={<span className="text-[10px] font-bold text-ui-primary">{formatEthDisplay(portfolio.totalEstimatedEth)}</span>}
              />
              <StudioMetricRow
                left={<span className="text-[10px] font-medium text-ui-secondary">Tracked Assets</span>}
                right={<span className="text-[10px] font-bold text-ui-primary">{totalAssetCount}</span>}
              />
              <StudioMetricRow
                left={<span className="text-[10px] font-medium text-ui-secondary">RWA Minted</span>}
                right={<span className="text-[10px] font-bold text-primary">{portfolio.typeCounts.rwa}</span>}
              />
              <StudioMetricRow
                left={<span className="text-[10px] font-medium text-ui-secondary">Receipts</span>}
                right={<span className="text-[10px] font-bold text-purple-300">{portfolio.typeCounts.receipts}</span>}
              />
              <StudioMetricRow
                left={<span className="text-[10px] font-medium text-ui-secondary">NFT Owned</span>}
                right={<span className="text-[10px] font-bold text-blue-300">{portfolio.typeCounts.nfts}</span>}
              />
            </div>
          </StudioPanel>
        </StudioSidebarScroll>
